...
from model.mamba_fusion import MambaFeatureFusion  # ← 이 줄 추가
...
class ParkingModel(nn.Module):
    def __init__(self, cfg: Configuration):
        ...
        self.mamba_fusion = MambaFeatureFusion(self.cfg)  # ← 이 줄 추가
    ...
    def encoder(self, data):
        ...
        mamba_feature = self.mamba_fusion(fuse_feature)  # ← 이 줄 추가
        pred_segmentation = self.segmentation_head(mamba_feature)  # ← fuse_feature를 mamba_feature로 변경
        
        return mamba_feature, pred_segmentation, pred_depth, bev_target  # ← fuse_feature를 mamba_feature로 변경

    def forward(self, data):
        mamba_feature, pred_segmentation, pred_depth, _ = self.encoder(data)  # fuse → mamba
        pred_control = self.control_predict(mamba_feature, data['gt_control'].cuda())  # fuse → mamba
