...
[!IMPORTANT] 4. [추가] Mamba Sequence Modeling: 트랜스포머에 의해 융합된 특징 시퀀스를 Mamba(SSM) 블록에 입력합니다. 병렬 스캔 (Parallel Scan) 기법을 사용하여 O(logL)의 복잡도로 병렬 처리를 수행합니다. S6(Selective SSM) 설계 시 입력 가중치($B$)와 출력 가중치($C$)를 공유하는 Simplified S6 구조를 적용하였다. 이를 통해 모델 파라미터를 효율화하여 학습 속도를 높였으며, 주행 맥락 정보를 제어 신호로 변환할 때의 일관성과 안정성을 극대화하였다. 트랜스포머가 포착한 정적 공간 특징들로부터 차량의 이동 흐름과 주행 이력을 반영한 동적 맥락(Driving Context)을 추출하여 최종 제어 헤드에 전달한다.
